...
void bno055_delay(int time) {  HAL_Delay(time);}
...
bno055_opmode_t bno055_getOperationMode() {
  bno055_opmode_t mode;
	uint8_t mod;
  bno055_readData(BNO055_OPR_MODE, &mod, 1);
	mode = mod;
  return mode;
}
...
bno055_vector_t bno055_getVector(uint8_t vec) {
  ...
  if (vec == BNO055_VECTOR_QUATERNION)
    {
		bno055_readData(vec, &buffer[0], 1);
		bno055_readData(vec+1, &buffer[1], 1);
		bno055_readData(vec+2, &buffer[2], 1);
		bno055_readData(vec+3, &buffer[3], 1);
		bno055_readData(vec+4, &buffer[4], 1);
		bno055_readData(vec+5, &buffer[5], 1); 
		bno055_readData(vec+6, &buffer[6], 1);
		bno055_readData(vec+7, &buffer[7], 1); 
		}
  else {
		bno055_readData(vec, &buffer[0], 1);
		bno055_readData(vec+1, &buffer[1], 1);
